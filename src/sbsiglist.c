...
typedef struct _siglist_context {
	int			verbose;

	const char		*infilename;
	const char		*outfilename;
	const cert_type *type;
	EFI_GUID		owner;
...
static int siglist_create(siglist_context *ctx)
{
	EFI_SIGNATURE_LIST *siglist;
	...
	size = sizeof(*siglist) + offsetof (EFI_SIGNATURE_DATA, SignatureData) + ctx->data_len;
	...
	siglist->SignatureSize = ctx->data_len + offsetof(EFI_SIGNATURE_DATA, SignatureData);
...
static cert_type *parse_type(const char *str)
{
	unsigned int i;
...
static void set_default_outfilename(siglist_context *ctx)
{
...
int main(int argc, char * const *argv)
{
	...
	ctx = talloc_zero(NULL, siglist_context);
