		...
		struct key_database *kdb)
{
	...
	struct keydb_add_ctx add_ctx;
	struct fs_keystore_entry *ke = NULL;
	int rc;
...
#else
static int check_efivars_mount(const char * __unused mountpoint)
#endif
{
#ifndef __APPLE__
		...
		const char *filename)
{
	...
	list_for_each(&keystore->keys, ke, keystore_list) {
		if (!strncmp(ke->name, filename, strlen(filename)))
        {
			return true;
        }
    }
...
{
	struct fs_keystore *keystore = NULL;
	unsigned int i;
...
static int find_new_keys(struct sync_context *ctx)
{
	...
	for (i = 0; i < ARRAY_SIZE(kdbs); i++ ) {
		struct fs_keystore_entry *ke = NULL;
		struct key *fs_key = NULL, *fw_key = NULL;
		bool found;
...
{
	struct fs_keystore_entry *ke = NULL, *ke_pk = NULL;
	int pks, rc;
